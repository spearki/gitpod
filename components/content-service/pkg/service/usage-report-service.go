...
func (us *UsageReportService) GetDownloadURL(ctx context.Context, req *api.GetDownloadURLRequest) (resp *api.GetDownloadURLResponse, err error) {
	span, ctx := opentracing.StartSpanFromContext(ctx, "UsageReport.GetDownloadURL")
	span.SetTag("name", req.Name)
	defer tracing.FinishSpan(span, &err)
	...
	return &api.GetDownloadURLResponse{
		Url: download.URL,
	}, nil
}
